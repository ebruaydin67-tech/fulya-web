...
const row = (label, value) =>
  value
    ? `<tr>
         <td style="padding:9px 0;color:#1f5e4c;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.07em;white-space:nowrap;vertical-align:top;">${escapeHtml(label)}</td>
         <td style="padding:9px 0 9px 18px;color:#141210;font-size:15px;vertical-align:top;">${escapeHtml(value)}</td>
       </tr>`
    : '';
...
<body style="margin:0;padding:0;background:#fbf6ea;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;">
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#fbf6ea;padding:24px 12px;">
    <tr><td align="center">
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:600px;background:#fffdf7;border:2px solid #141210;border-radius:20px;overflow:hidden;">
        ${inner}
        <tr><td style="background:#143f33;padding:22px 24px;">
          <p style="margin:0 0 8px;font-size:14px;font-weight:700;color:#fbf6ea;">${escapeHtml(CONTACT.name)}</p>
          <p style="margin:0;font-size:13px;color:rgba(251,246,234,.82);line-height:1.7;">
            ${escapeHtml(CONTACT.street)}, ${escapeHtml(CONTACT.city)}<br>
            Telefon: <a href="tel:${escapeHtml(CONTACT.phone.replace(/\s/g, ''))}" style="color:#f2c230;text-decoration:none;">${escapeHtml(CONTACT.phone)}</a><br>
            E-posta: <a href="mailto:${escapeHtml(CONTACT.email)}" style="color:#f2c230;text-decoration:none;">${escapeHtml(CONTACT.email)}</a>
          </p>
        </td></tr>
      </table>
      <p style="margin:16px 0 0;font-size:12px;color:#7c7268;">Bu e-posta başvurunuz üzerine otomatik olarak gönderilmiştir.</p>
    </td></tr>
  </table>
</body>
</html>`;
...
const header = (heading, sub, background = '#f2c230', color = '#141210') => `
  <tr><td style="background:${background};padding:24px;text-align:center;border-bottom:2px solid #141210;">
    ...
    <h1 style="margin:0;font-family:Georgia,'Times New Roman',serif;font-size:25px;font-weight:normal;color:${color};">${escapeHtml(heading)}</h1>
  ...
  const html = shell(
    ...
    header('Başvurunuz alındı', 'Fulya Akademi · Çocuk Kulübü') +
      `<tr><td style="padding:26px 24px;">
        <p style="margin:0 0 14px;font-size:16px;color:#141210;">Merhaba,</p>
        <p style="margin:0 0 22px;font-size:16px;color:#4a423a;line-height:1.65;">
          <strong style="color:#141210;">${escapeHtml(child)}</strong> için başvurunuzu aldık, teşekkür ederiz.
          Aşağıda başvuru bilgileriniz ve son adım olan banka havalesi yer alıyor.
        </p>

        <h2 style="margin:0 0 6px;font-family:Georgia,serif;font-size:18px;font-weight:normal;color:#141210;">Başvuru bilgileri</h2>
        ...
        <h2 style="margin:30px 0 10px;font-family:Georgia,serif;font-size:18px;font-weight:normal;color:#141210;">Son adım: banka havalesi</h2>
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#f5ecd8;border:2px solid #141210;border-radius:14px;">
        ...
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin-top:16px;background:rgba(242,194,48,.22);border:2px solid #e8a317;border-radius:14px;">
          <tr><td style="padding:16px 18px;">
            <p style="margin:0 0 6px;font-size:15px;font-weight:700;color:#141210;">Havale açıklaması nasıl yazılmalı?</p>
...
const statusUpdate = (r, status) => {
  ...
  const html = shell(
    ...
    header(
      ...
      isPaid ? '#1f5e4c' : '#f2c230',
      isPaid ? '#ffffff' : '#141210'
    ) +
      `<tr><td style="padding:26px 24px;">
        <p style="margin:0 0 14px;font-size:16px;color:#141210;">Merhaba,</p>
...
const internalNotice = (r) => {
  ...
  const html = shell(
    'Yeni başvuru',
    header('Yeni başvuru', `${r.group} · ${r.age} yaş`, '#1f5e4c', '#ffffff') +
